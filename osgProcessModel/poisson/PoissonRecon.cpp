...
PoissonRecon2020* PoissonRecon2020::instance = nullptr;
...
//{
int PoissonRecon2020::Poisson(std::string& inFileName, std::string outFileName)
{
	char* argv[5];
	//if (inFileName.length() <= 0) "cluster.ply";
	int argc = 5;

	char exeParam[10] = { '0' };
	char inputParam[10] = { '0' };
	char outputParam[10] = { '0' };
	char inputName[20] = { '0' };
	char outputName[20] = { '0' };
	memcpy(inputParam, "--in", 4);
	memcpy(outputParam, "--out", 5);
	memcpy(inputName, inFileName.c_str(), inFileName.length());
	memcpy(outputName, outFileName.c_str(), outFileName.length());
	argv[0] = exeParam;
	argv[1] = inputParam;
	argv[2] = inputName;
	argv[3] = outputParam;
