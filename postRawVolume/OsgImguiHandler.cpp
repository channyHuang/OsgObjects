...
OsgImGuiHandler::OsgImGuiHandler() : time_(0.f), mousePressed_{false}, mouseWheel_(0.f), initialized_(false) {
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
...
bool OsgImGuiHandler::handle(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa) {
    ...
    case osgGA::GUIEventAdapter::KEYUP:
    {
        ...
        if (special_key > 0) {
            assert((special_key > 0 && special_key < 512));
    ...
    {
        mouseWheel_ = (ea.getScrollingMotion() == osgGA::GUIEventAdapter::SCROLL_UP ? 1.f : -1.f);
        return wantCaptureMouse;
    }
    default:
        break;
