...
bool MeshPostProcessing::unwrap() {
    ...
    //auto vnormals = output_mesh.add_property_map<vertex_descriptor, Vector_3>("v:normals", CGAL::NULL_VECTOR).first;
    //auto fnormals = output_mesh.add_property_map<face_descriptor, Vector_3>("f:normals", CGAL::NULL_VECTOR).first;
    //CGAL::Polygon_mesh_processing::compute_normals(output_mesh, vnormals, fnormals);
    //pointWithNormals.resize(output_mesh.number_of_vertices());

    std::vector<float> position;

    for (vertex_descriptor vd : vertices(output_mesh)) {
        //pointWithNormals[vd].second = vnormals[vd];
        //pointWithNormals[vd].first = output_mesh.point(vd);
        ...
        for (uint32_t j = 0; j < mesh.chartCount; j++) {
            ...
            for (uint32_t k = 0; k < chart->faceCount; k++) {
                ...
                for (int l = 0; l < 3; l++) {
                    ...
                    positionColor[v.xref * 3 + l] = color[l];
                }
    ...
    if (1) {
        ...
        for (auto& f : output_mesh.faces()) {
            ...
            ofs << positionColor[idx * 3] << " " << positionColor[idx * 3 + 1] << " " << positionColor[idx * 3 + 2];
            ofs << std::endl;
        }
        ofs.close();
    }
...
bool MeshPostProcessing::unwrap(const std::string& sFileName, std::vector<std::pair<float, float>>& uvs) {
    std::vector<float> position;
/*
    osg::ref_ptr<osg::Geometry> geom = loadModelFile(sFileName);
    ...
    std::cout << varray->size() << " " << narray->size() << std::endl;
    indices.clear();

    for (int i = 0; i < geom->getNumPrimitiveSets(); ++i) {
        osg::PrimitiveSet* prim = geom->getPrimitiveSet(i);
        if (prim->getMode() != GL_TRIANGLES)
            continue;
        osg::DrawElementsUInt* drawElements = static_cast<osg::DrawElementsUInt*>(prim);
        std::cout << __FUNCTION__ << drawElements->getNumIndices() << std::endl;

        for (int i = 0; i < drawElements->getNumIndices(); ++i) {
            indices.push_back(drawElements->index(i) - 1);
        }

        break;
    }

    std::cout << position.size() << " " << indices.size() << std::endl;
    return false;
