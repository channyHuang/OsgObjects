...
inline unsigned int8_t *allocate_channel_data(unsigned int32_t size) {
#ifdef VOXEL_BUFFER_USE_MEMORY_POOL
    return VoxelMemoryPool::get_singleton()->allocate(size);
#else
    return (unsigned int8_t *)malloc(size * sizeof(unsigned int8_t));
#endif
}

inline void free_channel_data(unsigned int8_t *data, unsigned int32_t size) {
#ifdef VOXEL_BUFFER_USE_MEMORY_POOL
...
unsigned int32_t g_depth_bit_counts[] = {
	8, 16, 32, 64
};
unsigned int g_depth_max_values[] = {
	0xff, // 8
	0xffff, // 16
	0xffffffff, // 32
...
inline unsigned int32_t get_depth_bit_count(VoxelBuffer::Depth d) {
    //CRASH_COND(d < 0 || d >= VoxelBuffer::DEPTH_COUNT);
	return g_depth_bit_counts[d];
}

inline unsigned int get_max_value_for_depth(VoxelBuffer::Depth d) {
    //CRASH_COND(d < 0 || d >= VoxelBuffer::DEPTH_COUNT);
	return g_depth_max_values[d];
}

inline unsigned int clamp_value_for_depth(unsigned int value, VoxelBuffer::Depth d) {
	const unsigned int max_val = get_max_value_for_depth(d);
	if (value >= max_val) {
		return max_val;
	}
	return value;
...
static_assert(sizeof(unsigned int32_t) == sizeof(float), "unsigned int32_t and float cannot be marshalled back and forth");
static_assert(sizeof(unsigned int) == sizeof(double), "unsigned int and double cannot be marshalled back and forth");

inline unsigned int real_to_raw_voxel(real_t value, VoxelBuffer::Depth depth) {
	switch (depth) {
...
inline real_t raw_voxel_to_real(unsigned int value, VoxelBuffer::Depth depth) {
...
void VoxelBuffer::clear_channel(unsigned int channel_index, unsigned int clear_value) {
...
void VoxelBuffer::set_default_values(FixedArray<unsigned int, VoxelBuffer::MAX_CHANNELS> values) {
	for (unsigned int i = 0; i < MAX_CHANNELS; ++i) {
		_channels[i].defval = clamp_value_for_depth(values[i], _channels[i].depth);
	}
}

unsigned int VoxelBuffer::get_voxel(int x, int y, int z, unsigned int channel_index) const {
	...
	if (channel.data != nullptr) {
		const unsigned int32_t i = get_index(x, y, z);
		...
		switch (channel.depth) {
			...
			case DEPTH_16_BIT:
				return reinterpret_cast<unsigned int16_t *>(channel.data)[i];

			case DEPTH_32_BIT:
				return reinterpret_cast<unsigned int32_t *>(channel.data)[i];

			case DEPTH_64_BIT:
				return reinterpret_cast<unsigned int *>(channel.data)[i];
...
void VoxelBuffer::set_voxel(unsigned int value, int x, int y, int z, unsigned int channel_index) {
    //ERR_FAIL_INDEX(channel_index, MAX_CHANNELS);
	...
	if (do_set) {
		...
		switch (channel.depth) {
			...
			case DEPTH_16_BIT:
				reinterpret_cast<unsigned int16_t *>(channel.data)[i] = value;
				break;

			case DEPTH_32_BIT:
				reinterpret_cast<unsigned int32_t *>(channel.data)[i] = value;
				break;

			case DEPTH_64_BIT:
				reinterpret_cast<unsigned int *>(channel.data)[i] = value;
				break;
...
void VoxelBuffer::fill(unsigned int defval, unsigned int channel_index) {
    //ERR_FAIL_INDEX(channel_index, MAX_CHANNELS);
	...
	switch (channel.depth) {
		...
		case DEPTH_16_BIT:
			for (unsigned int32_t i = 0; i < volume; ++i) {
				reinterpret_cast<unsigned int16_t *>(channel.data)[i] = defval;
			}
			break;

		case DEPTH_32_BIT:
			for (unsigned int32_t i = 0; i < volume; ++i) {
				reinterpret_cast<unsigned int32_t *>(channel.data)[i] = defval;
			}
			break;

		case DEPTH_64_BIT:
			for (unsigned int32_t i = 0; i < volume; ++i) {
				reinterpret_cast<unsigned int *>(channel.data)[i] = defval;
			}
			break;
...
void VoxelBuffer::fill_area(unsigned int defval, Vector3i min, Vector3i max, unsigned int channel_index) {
    //ERR_FAIL_INDEX(channel_index, MAX_CHANNELS);
	...
	for (pos.z = min.z; pos.z < max.z; ++pos.z) {
		for (pos.x = min.x; pos.x < max.x; ++pos.x) {
			...
			switch (channel.depth) {
				case DEPTH_8_BIT:
					// Fill row by row
					memset(&channel.data[dst_ri], defval, area_size.y * sizeof(unsigned int8_t));
					break;

				case DEPTH_16_BIT:
					for (int i = 0; i < area_size.y; ++i) {
						((unsigned int16_t *)channel.data)[dst_ri + i] = defval;
					}
					break;

				case DEPTH_32_BIT:
					for (int i = 0; i < area_size.y; ++i) {
						((unsigned int32_t *)channel.data)[dst_ri + i] = defval;
					}
					break;

				case DEPTH_64_BIT:
					for (int i = 0; i < area_size.y; ++i) {
						((unsigned int *)channel.data)[dst_ri + i] = defval;
					}
					break;
...
template <typename T>
inline bool is_uniform(const unsigned int8_t *p_data, unsigned int32_t size) {
...
bool VoxelBuffer::is_uniform(unsigned int channel_index) const {
	...
	switch (channel.depth) {
		case DEPTH_8_BIT:
			return ::is_uniform<unsigned int8_t>(channel.data, volume);
		case DEPTH_16_BIT:
			return ::is_uniform<unsigned int16_t>(channel.data, volume);
		case DEPTH_32_BIT:
			return ::is_uniform<unsigned int32_t>(channel.data, volume);
		case DEPTH_64_BIT:
			return ::is_uniform<unsigned int>(channel.data, volume);
		default:
            //CRASH_NOW();
			break;
	}
...
void VoxelBuffer::compress_uniform_channels() {
	for (unsigned int i = 0; i < MAX_CHANNELS; ++i) {
		if (_channels[i].data != nullptr && is_uniform(i)) {
			...
			const unsigned int v = get_voxel(0, 0, 0, i);
			clear_channel(i, v);
		}
	}
}
			...
			if (channel.depth == DEPTH_8_BIT) {
				...
				for (pos.z = 0; pos.z < area_size.z; ++pos.z) {
					for (pos.x = 0; pos.x < area_size.x; ++pos.x) {
						...
						memcpy(&channel.data[dst_ri], &other_channel.data[src_ri], area_size.y * sizeof(unsigned int8_t));
					}
				}
				...
				for (pos.z = 0; pos.z < area_size.z; ++pos.z) {
					for (pos.x = 0; pos.x < area_size.x; ++pos.x) {
						...
						memcpy(&channel.data[dst_ri], &other_channel.data[src_ri], area_size.y * sizeof(unsigned int16_t));
					}
				}
				...
				for (pos.z = 0; pos.z < area_size.z; ++pos.z) {
					for (pos.x = 0; pos.x < area_size.x; ++pos.x) {
						for (pos.y = 0; pos.y < area_size.y; ++pos.y) {
							const unsigned int v = other.get_voxel(src_min + pos, channel_index);
...
*/
bool VoxelBuffer::get_channel_raw(unsigned int channel_index, ArraySlice<unsigned int8_t> &slice) const {
	const Channel &channel = _channels[channel_index];
	if (channel.data != nullptr) {
		slice = ArraySlice<unsigned int8_t>(channel.data, 0, channel.size_in_bytes);
		return true;
	}
	slice = ArraySlice<unsigned int8_t>();
	return false;
}

void VoxelBuffer::create_channel(int i, Vector3i size, unsigned int defval) {
	create_channel_noinit(i, size);
	fill(defval, i);
}

unsigned int32_t VoxelBuffer::get_size_in_bytes_for_volume(Vector3i size, Depth depth) {
...
void VoxelBuffer::create_channel_noinit(int i, Vector3i size) {
	Channel &channel = _channels[i];
	unsigned int32_t size_in_bytes = get_size_in_bytes_for_volume(size, channel.depth);
...
unsigned int32_t VoxelBuffer::get_depth_bit_count(Depth d) {
